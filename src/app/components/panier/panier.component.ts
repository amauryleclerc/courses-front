    ...
    exporter() {
        ...
        this.categories.forEach((categorie) => {
            ...
            this.panier.produitsSelect.filter((produitSelect) => produitSelect.produit.categorie.id === categorie.id)
                .map((element) => {
                    let text = element.produit.libelle;
                    if(element.promo){
                        text = text+"*";
                    }
                    if (element.quantite > 1 && !(element.unite && element.unite != "")) {
                        text =  text + " x";
                    }
                    if (element.quantite > 1) {
                        text =  text + " "+ element.quantite + " " +element.unite;
                    }
                    if(element.commentaire && element.commentaire !==""){
                        text = text + " - " + element.commentaire;
                    }
                    return { text: text };
                }).forEach((element) => {
